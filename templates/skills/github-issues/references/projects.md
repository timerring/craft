...
Manage GitHub Projects V2 only through `gh`. Use `gh project` where it exposes the required operation; use `gh api graphql` for field and item operations that require GraphQL.

## Workflow for project operations

1. **Find the project** — see [Finding a project by name](#finding-a-project-by-name) below
2. **Discover fields** — use `gh project field-list` or `gh api graphql` to get field and option IDs
3. **Find items** — use `gh project item-list` or `gh api graphql` to get item IDs
4. **Mutate** — use `gh project item-add`, `gh project item-edit`, `gh project item-delete`, or `gh api graphql`
...
2. **Discover fields** — run `gh project field-list PROJECT_NUMBER --owner OWNER --format json` to find the Status field and any Iteration field.

3. **Get all items** — run `gh project item-list PROJECT_NUMBER --owner OWNER --format json --limit LIMIT`. Increase the limit as needed.
...
Use `gh api graphql` to run GraphQL queries and mutations when `gh project` does not expose the required operation.
