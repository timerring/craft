...
Use `gh issue list` for repository-scoped listing, `gh search issues` for text or cross-repository search, and `gh api` only for advanced field queries unsupported by the higher-level commands.

## When to Use Search vs List vs Advanced Search

There are three `gh` routes:

| Capability | `gh issue list` | `gh search issues` | `gh api` advanced search |
|-----------|-----------------|--------------------|--------------------------|
| **Scope** | Single repository | Cross-repository and cross-organization | Cross-repository and cross-organization |
| **Issue field filters** | Host-dependent advanced search | Host-dependent advanced search | **Yes** with `advanced_search=true` |
| **Issue type filter** | `--type NAME` | Query qualifier | Query qualifier |
| **Boolean logic** | Through `--search` | Through query | **Yes** |
| **Label/state/date filters** | Yes | Yes | Yes |
| **Assignee/author/mentions** | Yes | Yes | Yes |
| **Text search** | Through `--search` | Yes | Yes |
| **Result limit** | Set with `--limit` | 1,000 maximum | 1,000 maximum |

**Decision guide:**
- **Single repo, simple filters:** use `gh issue list --repo OWNER/REPO`
- **Cross-repo or text search:** use `gh search issues`
...
- Results: max **1,000** total (use `gh issue list` with an appropriate limit for repository-scoped listing)
- Repo scan: searches up to **4,000** matching repositories
- Rate limit: **30 requests/minute** for authenticated search
- Issue field search requires `advanced_search=true` with `gh api` or `ISSUE_ADVANCED` with `gh api graphql`
